...
u8 _RTC_Init(void){
	...
	if(RTC_ReadBackupRegister(RTC_BKP_DR0)!=5050){//第一次配置
		...
		RTC_WriteBackupRegister(RTC_BKP_DR0,5050);//写入备份寄存器，标记已经初始化
	}
	return 0;
}
...
/**
  * @brief RTC日期设置（年、月、日、周）
  * @note 星期(1~7,0,非法!),年(0~99),月(1~12),日(0~31)
...
/**
  ...
  * @param 
  *      @arg week:星期几(1~7) @ref  RTC_Alarm_Definitions
  *      @arg hour,min,sec:小时,分钟,秒钟
  */
